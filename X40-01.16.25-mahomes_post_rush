...
# filter to only Mahomes rushes
  kcqb1 = pmdata1 %>%
    ...
    group_by(game_id, rusher_player_id, rusher_player_name, posteam) %>%
    summarize(rushes = n(),
...
# create plot 1
  pmplot1 = kcqb1 %>%
    ...
    ggplot(aes(x = game_id, y = rushes, group = 1)) +
    scale_x_discrete(label = game_labels) +
    scale_y_continuous(n.breaks = 8) +
    geom_abline(slope = 0, intercept = seq(c(1, 2, 3, 4, 5, 6, 7, 8, 9)),
                alpha = 0.45, color = "grey") +
    geom_line(color = "red", alpha = 0.8) + 
    geom_hline(yintercept = 4.5, linetype = "dashed", color = "blue") +
    geom_nfl_logos(aes(team_abbr = posteam, color = colorkc1), width = 0.06) +
    scale_color_identity() +
    annotate("text", label = "Mahomes O/U 4.5 Att.", x = 17, y = 4.65,
             fontface = "bold", color = "blue", size = 4.5) +
...
# create plot 2
  pmplot2 = ggplot(data = kcqb1, aes(x = rushes)) +
    ...
    geom_vline(xintercept = 4.5,
               linetype = "dashed", color = "blue", linewidth = 1.0) +
    scale_color_identity() +
    scale_x_continuous(n.breaks = 8) +
    labs(title = "Distribution of Mahomes Rush Att. by Playoff Game",
         caption = "By Nick Gasperi | @tbanalysis | Data @nflfastR",
         x = "Rush Attempts", y = "Frequency",
         tag = "00-0033873") +
    theme_minimal() +
    theme(legend.position = "none",
          plot.tag = element_nfl_headshot(size = 4.25, hjust = 1, vjust = 1),
