...
# plot data
  gim4plot = ggplot(data = gimmick4, aes(x = airyd, y = epa)) +
    geom_hline(yintercept = mean(gimmick4$epa), linetype = "dashed", color = "black") +
    geom_vline(xintercept = mean(gimmick4$att), linetype = "dashed", color = "black") +
    geom_nfl_logos(aes(team_abbr = posteam), width = 0.06, alpha = 0.7) +
    geom_text_repel(box.padding = 0.8, aes(label = gimmick4$passer_player_name, fontface = "bold", size = 12)) +
