...
# plot data
wk20rec1 = ggplot(data = wk20data,
                  aes(x = yacper, y = epaper)) +
  geom_hline(yintercept = mean(wk20data$epaper),
             linetype = "dashed",
             color = "black") +
  geom_vline(xintercept = mean(wk20data$yacper),
             linetype = "dashed",
             color = "black") +
  geom_nfl_logos(aes(team_abbr = posteam),      # replace points with team logos
                 width = 0.065,
                 alpha = 0.8) +
