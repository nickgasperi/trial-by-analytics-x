...
# plot data
  toandtdplot = ggplot(data = toandtd, aes(x = turnovers, y = tds)) +
    scale_x_reverse() +
    geom_hline(yintercept = mean(toandtd$tds), linetype = "dashed", color = "black") +
    geom_vline(xintercept = mean(toandtd$turnovers), linetype = "dashed", color = "black") +
    geom_smooth(method = "lm", se = FALSE, color = "grey") +
    geom_nfl_logos(aes(team_abbr = posteam, width = 0.06, alpha = 0.)) +
    geom_text_repel(box.padding = 0.7,
                    aes(label = toandtd$name, color = posteam)) +
