...
# plot data
  champplotrec = ggplot(data = champdatarec, aes(x = targets, epaper)) +
    geom_hline(yintercept = mean(champdatarec$epaper), linetype = "dashed", color = "grey30") +
    geom_hline(yintercept = 0, color = "chartreuse", alpha = 0.7) +
    geom_vline(xintercept = mean(champdatarec$targets), linetype = "dashed", color = "grey30") +
    geom_smooth(method = "lm", se = FALSE, color = "grey") +
    geom_nfl_logos(aes(team_abbr = posteam), width = 0.06, alpha = 0.8) +
    geom_text_repel(box.padding = 0.8,
                    aes(label = receiver_player_name, color = posteam, fontface = "bold")) +
