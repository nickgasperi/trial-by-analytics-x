...
## create a the plot for epa with the dataframe you just created
  plot_rpepa_2412 = ggplot(data = pbp2412_rp,
    ...
    geom_hline(yintercept = mean(pbp2412_rp$passperplayepa), linetype = "dashed", color = "red")+
    geom_vline(xintercept = mean(pbp2412_rp$rushperplayepa), linetype = "dashed", color = "red")+
    geom_nfl_logos(aes(team_abbr = defteam), width = .06, alpha = .8)+
    annotate("text", label = "Flores Zone", x = -0.27, y = 0, size = 6.5, color = "green", fontface = "bold.italic")+
    annotate("text", label = "Ravens", x = -0.2, y = 0.17, size = 6.5, color = "purple")+
    annotate("text", label = "Solid", x = -0.17, y = -0.07, size = 6.5, color = "green")+
    annotate("text", label = "Maybe Next Year", x = 0.02, y = 0.25, size = 6.5, color = "red")+
    theme_bw()+
...
## use the Files tab in the bottom right pane of RStudio to open and save image to desired file location
  ggsave("X Post 6 pt1 - epa allowed wk1-12.png", width = 14, height = 10, dpi = "retina")
...
## recreate the plot, but use yards/play this time
  plot_rpyd_2412 = ggplot(data = pbp2412_rp,
    ...
    geom_smooth(method = "lm", se = FALSE, color = "grey")+
    geom_hline(yintercept = mean(pbp2412_rp$passperplayyd), linetype = "dashed", color = "red")+
    geom_vline(xintercept = mean(pbp2412_rp$rushperplayyd), linetype = "dashed", color = "red")+
    geom_nfl_logos(aes(team_abbr = defteam), width = .06, alpha = .8)+
    annotate("text", label = "Great", x = 3.5, y = 5.2, size = 6.5, color = "green", fontface = "bold")+
    annotate("text", label = "Solid", x = 3.92, y = 5.15, size = 6.5, color = "green")+
    annotate("text", label = "Vulnerable", x = 3.3, y = 6, size = 6.5, color = "orange")+
    annotate("text", label = "Serviceable", x = 4.5, y = 5.4, size = 6.5, color = "orange")+
    annotate("text", label = "Unreliable", x = 4.8, y = 6.7, size = 6.5, color = "red")+
    annotate("text", label = "Fire Doug", x = 3.93, y = 7.06, size = 6.5, color = "red", fontface = "bold")+
    theme_bw()+
    theme(plot.title = element_text(face = "bold", hjust = 0.5, size = 20),
          plot.subtitle = element_text(face = "bold", hjust = 0.5, size = 14),
          axis.title = element_text(face = "bold", size = 14),
...
## export the plot
## use the Files tab in the bottom right pane of RStudio to open and save image to desired file location
  ggsave("X Post 6 pt2 - yds allowed wk1-12.png", width = 14, height = 10, dpi = "retina")
