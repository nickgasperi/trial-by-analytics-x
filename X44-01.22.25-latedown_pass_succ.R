...
# change play type to binary variable to calculate pass rate
  latedown = nfldata %>%
    filter(week == 20,
           ...
           qb_kneel == 0,
           !is.na(success)) %>%
    mutate(play_type = recode(play_type, "run" = 0, "pass" = 1)) %>%
    group_by(posteam) %>%
    summarize(plays = n(),
...
# plot data
  latedownp1 = ggplot(data = latedown, aes(x = passrate, y = sucrate)) +
    geom_hline(yintercept = mean(latedown$sucrate), linetype = "dashed", color = "red") +
    geom_vline(xintercept = mean(latedown$passrate), linetype = "dashed", color = "red") +
    geom_smooth(method = "lm", se = FALSE, color = "grey") +
    geom_nfl_logos(aes(team_abbr = posteam), width = 0.08, alpha = 0.8) +
    labs(title = "Late Down Pass Rate vs. Success Rate",
         subtitle = "2024 NFL Divisional Round",
         caption = "By Nick Gasperi | @tbanalysis | Data @nflfastR",
         x = "Pass Rate", y = "Success Rate") +
    theme_minimal() +
