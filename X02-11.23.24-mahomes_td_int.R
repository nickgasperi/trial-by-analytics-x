...
mahomesdata1 = nfldata15 %>%
  filter(passer_player_name == "P.Mahomes",          # include only passes thrown by Mahomes
         play_type == "pass",
         !is.na(air_yards)) %>% 
  group_by(passer_player_id,
           passer_player_name,
           season,
           posteam) %>%          # include team abbr. for later viz
  summarize(att = n(),
            td = sum(touchdown),
            int = sum(interception),
            tdint = td/int) %>%                   # create TD:Int Ratio for ggplot()
  mutate(tdint = round(tdint, digits = 1)) %>%    # round TD:Int Ratio to 1 decimal
  print(n = Inf)                                    
...
mahomesratioplot = ggplot(data = mahomesdata1, 
  ...
  labs(title = "Mahomes Touchdown to Interception Ratio By Year",
       ...
       tag = "00-0033873") +                    # adding tag for player image added in theme()
