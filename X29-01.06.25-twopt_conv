...
# select only playoff teams
  twoptplot = ggplot(subset(usedata, posteam %in% playoffteam), aes(x = attperg, y = sucrate)) +
    geom_hline(yintercept = mean(usedata$sucrate), linetype = "dashed", color = "red") +
    geom_vline(xintercept = mean(usedata$attperg), linetype = "dashed", color = "red") +
    geom_nfl_logos(aes(team_abbr = posteam), alpha = 0.8, width = 0.07) +
    geom_text_repel(box.padding = 1.0, aes(label = posteam, color = posteam, fontface = "bold.italic", size = 2)) +
    scale_color_nfl() +
    labs(title = "2024 NFL Playoff Teams | 2-Pt Conversions",
         caption = "By Nick Gasperi | @tbanalysis | Data @nflfastR",
         x = "Attempts Per Game", y = "% Successful") +
    theme_minimal() +
