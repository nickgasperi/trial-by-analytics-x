...
# change field goal result variable to binary
  kickdata = nfldata8 %>%
...
# plot data
  kickplot1 = kickdata %>%
    mutate(color4 = ifelse(season == 2024, posteam, "b/W")) %>%
    mutate(label4 = ifelse(season == 2024, kicker_player_name, "")) %>%
    ggplot(aes(x = att, y = pctmade)) +
    geom_hline(yintercept = mean(kickdata$pctmade), linetype = "dashed") +
    geom_vline(xintercept = mean(kickdata$att), linetype = "dashed") +
