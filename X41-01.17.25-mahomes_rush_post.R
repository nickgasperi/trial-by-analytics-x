...
# create plot
  pmplot3 = kcqb1 %>%
    ...
    ggplot(aes(x = game_id, y = rushyd, group = 1)) +
    scale_x_discrete(label = game_labels) +
    scale_y_continuous(n.breaks = 8) +
    geom_line(color = "red", alpha = 0.8) + 
    geom_hline(yintercept = 24.5, linetype = "dashed", color = "blue") +
    geom_hline(yintercept = 29.1, linetype = "dashed", color = "darkgreen") +
    geom_nfl_logos(aes(team_abbr = posteam, color = colorkc1), width = 0.06) +
    scale_color_identity() +
    annotate("text", label = "O/U 24.5 Yd", x = 16, y = 26.2,
             fontface = "bold", color = "blue", size = 4.5) +
    annotate("text", label = "Avg. = 29.1 Yd", x = 12.5, y = 30.8,
             fontface = "bold", color = "darkgreen", size = 4.5) + 
