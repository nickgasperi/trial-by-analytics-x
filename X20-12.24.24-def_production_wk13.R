...
# plot data
recentplot1 = ggplot(data = recentdata1, aes(x = eparush, y = epapass)) +
  geom_hline(yintercept = mean(recentdata1$epapass), linetype = "dashed", color = "red") +
  geom_vline(xintercept = mean(recentdata1$eparush), linetype = "dashed", color = "red") +
  geom_smooth(method = "lm", se = FALSE, color = "grey") +
  geom_nfl_logos(aes(team_abbr = defteam), width = 0.07, alpha = 0.8) +
  scale_x_reverse() +
  scale_y_reverse() +
  labs(title = "EPA Per Rush vs. EPA Per Pass Allowed",
       subtitle = "2024 NFL Defenses | Wk 13-16",
       caption = "By Nick Gasperi | @tbanalysis | Data @nflfastR",
       x = "EPA Per Rush", y = "EPA Per Pass") +
  theme_bw() +
